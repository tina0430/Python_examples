...
def setNewAddress(store):
    ...
    # print(table)
#     table[table['sido' == '00']]
    table = table.drop(table.index[1162])
    table = table.drop(table.index[798])
#     table = table[table['sido'] =='00']
    ...
    gungu_alias = """고양시일산서구:고양시 고양시덕양구:고양시 고양시일산동구:고양시
                    ...
                    포항시북구:포항시 포항시남구:포항시
                    창원시마산회원구:창원시 마산회원구:창원시 창원시진해구:창원시 진해시:창원시
...
#     m = pd.merge(table, sido_table, on=['sido', 'gungu'], how='outer',suffixes=['', '_'], indicator=True)
